...
var _ = Describe("Cluster create/delete flow", Ordered, func() {
	...
	Context("Metrics are generated", func() {
		It("Should return 200 on /metrics", func() {
			...
			Expect(string(body)).To(ContainSubstring("cluster_manager_http_response_codes_counter{code=\"204\",method=\"DELETE\",path=\"/v2/templates/baseline-k3s/v0.0.5\"}"))
			Expect(string(body)).To(ContainSubstring("cluster_manager_http_response_codes_counter{code=\"409\",method=\"DELETE\",path=\"/v2/templates/baseline-k3s/v0.0.5\"}"))
