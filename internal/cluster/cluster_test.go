...
func TestNodes(t *testing.T) {
	...
	// mock k8s client
	cli := k8s.NewMockClient(t)

	// Directly mock the GetMachines method
	cli.On("GetMachines", mock.Anything, namespace, clusterName).Return(machines, nil)

	// intel machines
	for _, im := range intelMachines {
		// Directly mock IntelMachine method for each machine
		cli.On("IntelMachine", mock.Anything, namespace, im.Name).Return(im, nil)
	}
